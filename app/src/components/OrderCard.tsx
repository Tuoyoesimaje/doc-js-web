...
export default function OrderCard({ order }: OrderCardProps) {
  const statusConfig = {
    ...
    processing: {
      color: 'bg-warning-100 text-warning-800 border-warning-200 dark:bg-warning-900/30 dark:text-warning-400 dark:border-warning-800',
      icon: (
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          <circle cx="8" cy="8" r="6" stroke="currentColor" strokeWidth="2"/>
    ...
    ready: {
      color: 'bg-primary-100 text-primary-800 border-primary-200 dark:bg-primary-900/30 dark:text-primary-400 dark:border-primary-800',
      icon: (
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
          <circle cx="8" cy="8" r="6" stroke="currentColor" strokeWidth="2"/>
    ...
    delivered: {
      color: 'bg-accent-100 text-accent-800 border-accent-200 dark:bg-accent-900/30 dark:text-accent-400 dark:border-accent-800',
      icon: (
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
  ...
  return (
    <Link to={`/orders/${order.id}`}>
      <motion.div
        ...
        className="bg-white dark:bg-gray-800 rounded-2xl border-2 border-gray-100 dark:border-gray-700 p-6 hover:border-primary-600 dark:hover:border-primary-500 hover:shadow-xl transition-all duration-300 cursor-pointer"
      >
          ...
          <div className="flex-1 min-w-0">
            ...
            {order.address && (
              <div className="flex items-start gap-2 text-sm text-gray-600 dark:text-gray-400">
                <svg width="18" height="18" viewBox="0 0 18 18" fill="none" className="text-gray-400 dark:text-gray-500 mt-0.5 flex-shrink-0">
          ...
          <div className="text-right flex-shrink-0">
            ...
            <div className="flex items-center gap-1.5 justify-end">
              {order.payment_status === 'confirmed' ? (
                <>
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="text-accent-600 dark:text-accent-400">
                    <circle cx="7" cy="7" r="6" stroke="currentColor" strokeWidth="1.5"/>
                    <path d="M4 7l2 2 4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                  <span className="text-xs font-bold text-accent-600 dark:text-accent-400">Paid</span>
                </>
              ) : (
                <>
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="text-warning-600 dark:text-warning-400">
                    <circle cx="7" cy="7" r="6" stroke="currentColor" strokeWidth="1.5"/>
                    <path d="M7 4v3M7 10h.01" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                  </svg>
                  <span className="text-xs font-bold text-warning-600 dark:text-warning-400">Pending</span>
                </>
              )}
            </div>
          </div>
        ...
        {/* View Details Arrow */}
        <div className="mt-4 pt-4 border-t-2 border-gray-100 dark:border-gray-700 flex items-center justify-between">
